...
eval (Loop e1) env = u
  where
    u = orelse (eval e1 env $> (\_ -> u)) (result Nil)
...
obey (Calculate exp) (env, mem) =
  let (mv, mem') = eval exp env mem in
  case mv of
    Just v -> (print_value v, (env, mem'))
    Nothing -> ("***exit in main program***", (env, mem'))
obey (Define def) (env, mem) =
  let x = def_lhs def in
  let (menv', mem') = elab def env mem in
  case menv' of
    Just env' -> (print_defn env' x, (env', mem'))
    Nothing -> ("***exit in definition***", (env, mem'))
