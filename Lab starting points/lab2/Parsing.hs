...
module Parsing(nextch, switch, star, incln, make_kwlookup,
                ...
                print_value, print_defn, Wrappable(..),
                dialog, dialogm, printStrLn, Parser, Syntax) where
...
execute g x dflt =
  Control.Exception.catch (g x) handler
  where
    handler e =
      let msg = takeWhile (/= '\n')
                      (show (e :: Control.Exception.ErrorCall)) in
      do hPutStrLn stderr ("Error: " ++ msg); return dflt
...
-- Paradoxically, defining |primwrap| involves precisely the kind of
-- interaction between language features, in this case lazy evaluation
-- and exceptions, that the book shows is tricky to get right.  That's
-- why the |Wrappable| class and its two overlapping instances is needed.

primwrapper :: Show a => String -> [a] -> b -> b
primwrapper prim args res =
  unsafePerformIO 
    (Control.Exception.catch (return $! res) match_fail)
  where 
...
class Wrappable t where
  primwrap :: String -> t -> t

instance Show a => Wrappable ([a] -> b) where
  primwrap x f args = primwrapper x args (f args)

instance {-# OVERLAPPING #-} Show a => Wrappable ([a] -> b -> c) where
  primwrap x f args y = primwrapper x args (f args y)
